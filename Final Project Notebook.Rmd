...
# use the binary meds or proc variable, we get 0.817
```{r}
#fit model
boost = gbm(death~.,data=train,distribution='bernoulli',n.trees=1000,shrinkage=0.01)
...
```{r}
#fit model
boost = gbm(death~.,data=train.balanced,distribution='bernoulli',n.trees=1000,shrinkage=0.01)
#get predicted probabilities
train.probs = predict(boost, train.balanced, n.trees = 1000)
val.probs = predict(boost, validation.onehot, n.trees = 1000)
#get binary predictions
train.pred=ifelse(train.probs>0.5, 1, 0)
val.pred=ifelse(val.probs>0.5, 1,0)
#get accuracy 
...
set.seed(1)
n=nrow(joined_df)
train.i=sample(1:n, n*0.75)
train=joined_df[train.i,]
validation=joined_df[-train.i,]
```
