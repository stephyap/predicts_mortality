...
## steph added (for PCA the unsupervised learning):
validationset = validation %>% select(-death)
```


### Dimension reduction

1. Simple Linear Regression (Yi Yao) 

2. PCA 
```{r}
pca_train = train %>% select(-death)
pca_validationset = validationset

pca_train = as.data.frame(sapply(pca_train, as.numeric))
pca_validationset = as.data.frame(sapply(pca_validationset, as.numeric))
...
```{r}
pr_var ## eliminate tho variables that didn’t have an eigenvalue greater than 1 (last one being 81st)
```

```{r}
#Plot
plot( prop_varex, xlab = "Principal Component", 
                  ylab = "Proportion of Variance Explained", type = "b" , xlim=c(0,50))
```
...
```{r}
# Create new dataset based on the PCA result aboe
pca_new_train = data.frame(death=train$death, pca$x)
t = as.data.frame(predict(pca, newdata=pca_validationset))

new_train = pca_new_train[, 1:82]
new_validationset = t[, 1:81]
```

# fit logistic reg model on new PCA training data
```{r}
glm.fit=glm(death~., data=new_train, family=binomial)
```
# get predictions on PCA validation set (83.6%)
```{r}
glm.probs=predict(glm.fit, newdata=new_validationset, type="response") 
...
#confusion matrix
    ...
    PCA True Positive Rate = 62.8%
    PCA False Positive Rate = 13.8%
```{r}
table(glm.pred, validation$death)
```

# AUC for logistic reg model after PCA dimension reduction
```{r}
labels=factor(validation$death)
auc(roc(glm.probs,labels))
plot(roc(glm.probs,labels))
```

```{r}
roc_obj <- roc(validation$death, glm.pred)
auc(roc_obj)
```
