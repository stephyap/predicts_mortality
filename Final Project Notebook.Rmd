...
```{r}
train <- train %>% filter(gender != "Unknown")
...
# k-fold cross validation
cv.error=cv.glm(train,glm.fit_reduced,K=5)$delta[1]
cv.error
#get predictions on validation set
...
##### with reduced variables set 2 from forward.res.RData
```{r}
...
```{r}
train.reduced <- train %>% select(red.var,death)
validation.reduced <- validation %>% select(red.var,death)
glm.fit_reduced=glm(death~., data=train.reduced, family=binomial)
# k-fold cross validation
cv.error=cv.glm(train,glm.fit_reduced,K=5)$delta[1]
cv.error
...
##### with reduced variables set 3 from forward.res.RData
```{r}
red.var_3 <- c("age", "P44", "P154650", "P2549", "P2548", "P161794", "P161793", 
"P4852", "ocular_lubricant", "mannitol", "cefepime", "lvp_solution", 
"P337", "P620", "glucose")
```

    CV error rate 0.1362562
    Accuracy 0.7890879
...
```{r}
train.reduced <- train %>% select(red.var_3,death)
validation.reduced <- validation %>% select(red.var_3,death)
glm.fit_reduced=glm(death~., data=train.reduced, family=binomial)
# k-fold cross validation
cv.error=cv.glm(train,glm.fit_reduced,K=5)$delta[1]
cv.error
