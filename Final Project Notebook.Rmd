...
### BB - pivot med and proc dfs and merge with joined_df to get individual meds/procedures as features
...
# First run accuracy = 81.0%
# Using binary meds and procs, accurcy is 83.2%
...
edited_var <- c("141", "44", "norepinephrine", "age", "2549", "ondansetron", 
"154650", "nimodipine", "122879", "2548", "any_vaso", "piperacillin-tazobactam", 
"hydralazine", "heparin", "lidocaine", "ocular lubricant", "dopamine", 
"fentanyl", "glucose", "n_vaso", "docusate", "labetalol", "etomidate", 
"mannitol", "min_days_med_adm", "43", "4852", "codeine", "hydromorphone", 
"sodium bicarbonate", "lvp solution with hypertonic saline", 
"cefepime", "310", "4631", "3006", "lorazepam", "acetaminophen-oxycodone", 
"chlorhexidine topical", "succinylcholine", "apap/butalbital/caffeine", 
"161794", "161793", "min_days_pro_adm", "4756", "sterile water", 
"albuterol-ipratropium", "ipratropium", "141780", "protamine", 
"levofloxacin", "n_proc", "acetaminophen-hydrocodone", "levetiracetam", 
"vancomycin", "metoclopramide", "potassium phosphate-sodium phosphate", 
"ceftriaxone", "promethazine", "nicardipine", "max_days_med_adm", 
"337", "lvp solution with potassium", "lvp solution", "glucagon", 
"vecuronium", "oxycodone", "2232", "glycopyrrolate", "dexamethasone", 
"calcium chloride", "42", "sodium chloride", "diphenhydramine", 
"propofol", "ketorolac", "157534", "magnesium sulfate", "phytonadione", 
"acetaminophen", "digoxin", "neostigmine", "verapamil", "356", 
"simvastatin", "ranitidine", "159871", "284", "albuterol", 
"methylprednisolone", "pravastatin", "insulin regular", "midazolam", 
"meperidine", "sugammadex", "morphine", "clindamycin", "senna", 
"furosemide", "5097", "606", "2610", "aspirin", "phenytoin", 
"remifentanil", "papaverine", "insulin aspart-insulin aspart protamine", 
"90", "calcium carbonate", "hydrochlorothiazide", "clonidine", 
"docusate-senna", "indocyanine green", "iopamidol", "155690", 
"prochlorperazine", "157536", "acetaminophen-codeine", "nicotine", 
"n_med", "calcium gluconate", "157076", "620", "race", "sufentanil", 
"insulin lispro", "bisacodyl", "iodixanol", "eptifibatide", "losartan", 
"159447", "zolpidem", "158020", "157074", "lisinopril", "110148", 
"esmolol", "dolasetron", "al hydroxide/mg hydroxide/simethicone", 
"gender", "84", "heparin flush", "fluticasone nasal", "amlodipine", 
"98597", "cefazolin", "rosuvastatin", "lidocaine topical")
```

```{r}
train.reduced <- train %>% select(edited_var,death)
validation.reduced <- validation %>% select(edited_var,death)
glm.fit_reduced=glm(death~., data=train.reduced, family=binomial)

#get predictions on validation set
glm.probs=predict(glm.fit_reduced, newdata=validation.reduced, type="response") 
glm.pred=ifelse(glm.probs>0.5, 1, 0)
val.accuracy = mean(glm.pred==validation.reduced$death)
val.accuracy
#confusion matrix
table(glm.pred, validation.reduced$death)
```

```{r}
roc_obj <- roc(validation.reduced$death, glm.pred)
auc(roc_obj)
```

<<<<<<< HEAD
#### Begin BB modeling section #####
=======
3. Gradient Boosting
# first run with n.trees=1000 and shrinkage=0.01, accuracy = 0.806
# however, 80% of actual outcomes are 0, so this is not a skillful model
# likely need to balance the classes between train and test sets to get better results
>>>>>>> f2a63f9c9de30a68dd5391391b08f009c6a8ff28
...
```{r}
#fit model
boost = gbm(death~.,data=train,distribution='bernoulli',n.trees=1000,shrinkage=0.01, interaction.depth=4,
            class.stratify.cv = 5)
...
for(i in 1:length(n.features)){
  ...
  boost.auc[i] = auc(roc(val.probs,labels))
}
...
train.balanced = SMOTE(X=train.onehot[,-8],target=train.onehot[,8])
...
# accuracy = 76.3%
# positive class recall increased from 38% to 66% by using class balanced rather than raw data
```{r}
train.balanced$death = as.numeric(train.balanced$death)

#fit model on training data
glm.fit=glm(death~., data=train.balanced, family=binomial)

#get predictions on validation set
glm.probs=predict(glm.fit, newdata=validation.onehot, type="response") 
glm.pred=ifelse(glm.probs>0.5, 1, 0)
val.accuracy = mean(glm.pred==validation.onehot$death)
val.accuracy
#confusion matrix
table(glm.pred, validation.onehot$death)
```
...
>>>>>>> f2a63f9c9de30a68dd5391391b08f009c6a8ff28
```{r}
#set values for mtry (number of predictors sampled for spliting at each node)
p = dim(train)[2]-1 #p=13
p2 = p/2
p_sqrt = sqrt(p)

<<<<<<< HEAD
# create x and y training and test sets
x.train = train.balanced[,-244]
y.train = as.factor(train.balanced$death)
x.val = validation.onehot[,-244]
y.val = as.factor(validation.onehot$death)

#fit random forest for each value of mtry
rf.p=randomForest(x.train, y.train, xtest = x.val, ytest = y.val, mtry = p, ntree = 100)
rf.p2=randomForest(x.train, y.train, xtest = x.val, ytest = y.val, mtry = p2, ntree = 100)
rf.p_sqrt=randomForest(x.train, y.train, xtest = x.val, ytest = y.val, mtry = p_sqrt, ntree = 100)

#plot test error for each model
plot(1:100, rf.p$test$err.rate[,1], col = "red", type = "l", xlab = "Number of Trees", ylab = "Test Error Rate", ylim = c(.15, .30))
lines(1:100, rf.p2$test$err.rate[,1], col = "blue", type = "l")
lines(1:100, rf.p_sqrt$test$err.rate[,1], col = "green", type = "l")
...
4. KNN
```{r}
train1=as.matrix(train$n_med)
validation1=as.matrix(validation$n_med)
set.seed(1)
knn_pred=knn(train1, validation1, train$death, k=1, prob=T, use.all = FALSE)
table(knn_pred, validation$death)
mean(knn_pred == validation$death)
```

5. LDA
```{r}
lda_model = lda(death~., family=binomial, data=train)
lda_pred = predict(lda_model, validation, type = "response")
confusion_matrix = table(lda_pred$class, validation$death)
print(confusion_matrix)
mean(lda_pred$class == validation$death)
```

6. QDA
```{r}
qda_model = qda(death~n_med+n_proc, family=binomial, data=train)
qda_pred = predict(qda_model, validation, type="response")
# qda_model
confusion_matrix = table(qda_pred$class, validation$death)
print(confusion_matrix)
mean(qda_pred$class == validation$death)
```

7..Tree
```{r}
tree_model=tree(as.factor(death)~.,joined_df)
summary(tree_model)
plot(tree_model)
text(tree_model,pretty=0)
tree_model
train.pred = predict(tree_model, train, type="class")
val.pred = predict(tree_model, validation,type="class")
#get binary predictions
confusion_matrix = table(train.pred, train$death)
print(confusion_matrix)
confusion_matrix1 = table(val.pred, validation$death)
print(confusion_matrix1)
mean(train.pred==train$death)
mean(val.pred==validation$death)

```

8. Bagging
```{r}
bag=randomForest(as.factor(death)~n_vaso, data=train, mtry=9, ntree=100, importance=TRUE)
plot(bag)
bag
train.p = predict(bag, train, type="prob")
# train.p
val.p = predict(bag, validation, type= "prob")
# val.p
#predictions
train.pred=ifelse(train.p>0.5, 1, 0)
val.pred=ifelse(val.p>0.5, 1,0)
#accuracy
mean(train.pred==train$death)
mean(val.pred==validation$death)
```
...
9. Random forest
```{r}
rf=randomForest(as.factor(death)~.,data=train, mtry=3, n.tree=100)
rf
train.p = predict(rf, train,type= "prob")
val.p = predict(rf, validation,type= "prob")
...
# compute the final lasso model using lambda.min (Accuracy: 0.8198198):
```{r}
...
# Lasso with lambda.min (AUC = 0.5729)
```{r}
roc_obj <- roc(test.data$death, predicted.classes)
auc(roc_obj)
```

# compute the final model using lambda.lse (Accuracy: 0.8098098):
```{r}
...
# Lasso with lambda.lse (AUC = 0.531)
```{r}
roc_obj <- roc(test.data$death, predicted.classes)
auc(roc_obj)
```
