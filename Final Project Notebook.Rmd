...
medication_train <- medication_train %>% 
  mutate(
    med_since_adm = as.numeric(difftime(medication_date, admission_date, units ="days"))
  )
```
...
procedure_train <- procedure_train %>% 
  mutate(
    pro_since_adm = as.numeric(difftime(procedure_date, admission_date, units ="days"))
  )
```

#### Combining all dataframes into one by id and admission date
```{r}
