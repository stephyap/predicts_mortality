...
```{r}
set.seed(3)
knn_pred=knn(train, validation, train$death, k=1, prob=T, use.all = FALSE)
table(knn_pred, validation.reduced$death)
mean(knn_pred == validation.reduced$death)
```
...
```{r}
set.seed(1)
sl = SuperLearner(X=train.reduced$death, Y=train.reduced, family = binomial(), SL.library = "SL.glm")
```
#Individual model for gbm
```{r}
set.seed(1)
sl = SuperLearner(X=train$death, Y=train, SL.library = "SL.gbm")
```
#Multiple model fit
```{r}
set.seed(1)
sl = SuperLearner(X=train$death, Y=train, family = binomial(),
