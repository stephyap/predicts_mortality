...
#get predicted probabilities
train.probs = predict(boost.test, train_final[,c('death',boost.top.features[1:70])], n.trees = 1000, type='response')
#get binary predictions
train.pred=ifelse(train.probs>0.5, 1, 0)
#get accuracy 
train.accuracy = mean(train.pred==train_final$death)
train.accuracy
#confusion matrix
table(train.pred, train_final$death)
#auc
#plot(roc(val.probs,labels))
labels=factor(train_final$death)
pROC::auc(roc(labels,train.probs))



#make predictions on test set using full boost model
test.probs = predict(boost.test, test_df[,c(-1)], n.trees = 1000, type='response')
#make preds on test set using reduced boost model with n=70
#make final df with patient id and prediction
final_preds = data.frame(patient_id = test_df[,c(1)], pred = test.probs)
#write to csv
