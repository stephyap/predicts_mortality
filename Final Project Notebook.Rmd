...
5. Statistics in age  
    ...
    for patients who had Death == TRUE
    
```{r}
demo_train %>% filter(death==TRUE) %>% summary(age)
```

    for patients who had Death == FALSE
    
```{r}
demo_train %>% filter(death==FALSE) %>% summary(age)
```


6. Descriptive statistics for # of days between discharge - admission

```{r}
summary(demo_train$days_dis_adm)
```
