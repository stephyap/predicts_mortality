...
Should we combine Unknown into Others? //Steph: I don't think so, I would think evenly distribute the Unknown into all the categories we have is better than dumping all of it to Others. What does the rest think? // Yi: That sounds like a crude imputation. Let's try a imputation model.
...
If we simply delete the unknown values:

```{r}
demo.tmp3 <- demo_train_new %>% filter(race != "Unknown")
cg.new3 <- compareGroups(death ~ gender+race+age+los, 
                    data = demo.tmp3, 
                    method = c(3,3,1,2), 
                    Q1 = 0, Q3 = 1)
createTable(cg.new3, show.n = F, show.p.overall = T)
```
Conclusion: delete the Unknowns.
