...
#### 2. Logistic Regression (with Reduced variables)
...
```{r}
reduced <- c("P2548", "P44", "norepinephrine", "age", "P2549", "P122879", 
"P154650", "P161794", "hydralazine", "fentanyl", "P4852", "mannitol", 
"propofol", "ocular_lubricant", "nimodipine", "lvp_solution", 
"P337", "lidocaine", "iopamidol", "lvp_solution_with_hypertonic_saline", 
"piperacillin_tazobactam", "P284", "P4631", "P157534", "calcium_carbonate", 
"docusate_senna", "cefazolin", "glucose", "acetaminophen_oxycodone")
```

```{r}
train.reduced <- train %>% select(reduced,death)
validation.reduced <- validation %>% select(reduced,death)

default_glm_mod_red=train(
  form=death~.,
  data=train.reduced,
  trControl=trainControl(method="cv", number=5),
  method="glm",
...
# AUC 
glm.probs=predict(default_glm_mod_red, newdata=validation.reduced) ## probability
roc_obj <- roc(validation.reduced$death, glm.probs)
auc = auc(roc_obj)
cat('AUC', auc, '\n')

# Accuracy 
glm.pred=ifelse(glm.probs>0.5, 1, 0)
val.accuracy = mean(validation.reduced$death == glm.pred)
cat('Accuracy', val.accuracy)
#confusion matrix
...
cat('Lambda.1se: ', cv.lasso$lambda.1se, '\n')
cat('Lambda.min: ', cv.lasso$lambda.min)
```
...
SL.rf.better = function(...) {
  SL.randomForest(..., num.trees = 500, mtry = 6)
}

SL.xgb.better = function(...) {
  SL.xgboost(..., n.trees = 1000, max_depth = 4, eta = 0.01, subsample = 0.5)
}

SL.gbm.better = function(...) {
  SL.gbm(..., n.trees = 1000, max_depth = 4, eta = 0.01, subsample = 0.5)
}

# SL.xgb.better2 = function(...) {
#   SL.xgboost(..., n.trees = 1000, max_depth = 2, shrinkage = 0.01, subsample = 1)
# }
...
SL.ksvm.better = function(...) {
  SL.ksvm(..., type = "C-svc", # classification
          kernel = "rbfdot", # linear kernel
          C = 5, # cost = 1
...
```{r}
cv.sl.all1 = CV.SuperLearner(Y = y.train, X = x.train.full, 
                            family = binomial(),
                          V = 5,
                          method = "method.AUC",
                          SL.library = c("SL.glmnet.better", 
                                         "SL.rf.better", 
                                         "SL.xgb.better", 
                                         "SL.ksvm.better"
                                         ))
save(cv.sl.all1, file = "cv.sl.all1.RData")
```

```{r}
sl.all1.pred = predict(sl.all1, x.valid.full, onlySL = F)
str(sl.all1.pred)
```

```{r fig.height=4, fig.width=4}
sl.en.roc <- rocit(score = sl.all1.pred$library.predict[,1], class = y.valid)
sl.rf.roc <- rocit(score = sl.all1.pred$library.predict[,2], class = y.valid)
sl.xgb.roc <- rocit(score = sl.all1.pred$library.predict[,3], class = y.valid)
sl.svm.roc <- rocit(score = sl.all1.pred$library.predict[,4], class = y.valid)
sl.roc <- rocit(score = sl.all1.pred$pred[,1], class = y.valid)
```

```{r}
# colors = c('#66c2a5','#fc8d62','#8da0cb','#e78ac3')
png("sl_roc_plot_v2.png", height = 600, width = 600)
colors = c('#1f78b4','#33a02c','#e31a1c','#ff7f00','#6a3d9a')
plot(sl.en.roc, values = T, YIndex = F, legend = F, col = colors[1], lwd = 1.5)
par(new = T)
plot(sl.rf.roc, values = T, YIndex = F, legend = F, col = colors[2], 
     xaxt = "n", yaxt = "n", xlab = "", ylab = "", lwd = 1.5)
par(new = T)
plot(sl.xgb.roc, values = T, YIndex = F, legend = F, col = colors[3], 
     xaxt = "n", yaxt = "n", xlab = "", ylab = "", lwd = 1.5)
par(new = T)
plot(sl.svm.roc, values = T, YIndex = F, legend = F, col = c(colors[4]), 
     xaxt = "n", yaxt = "n", xlab = "", ylab = "", lwd = 1.5)
par(new = T)
plot(sl.roc, values = T, YIndex = F, legend = F, col = c(colors[5], "#BEBEBE"), 
     xaxt = "n", yaxt = "n", xlab = "", ylab = "", lwd = 1.5)
all.auc = round(c(sl.en.roc$AUC, sl.rf.roc$AUC, sl.xgb.roc$AUC, sl.svm.roc$AUC, sl.roc$AUC),3)
legend("bottomright", legend = paste0(c("EN", "RF", "XGB", "SVM", "SL"), " AUC = ", all.auc), col = colors, lwd = 2, cex = 2, 
       bty = "n")
dev.off()
```
...
# 10 fold CV results: best performance: 0.1790113 

svm.linear <- svm(death ~ ., data = train, kernel = "linear", cost = 0.01,decision.values=T, probability =T)
...
# radial:
# single
# 10 fold CV error from on the training data: 0.1795731

svm.radial <- svm(death ~ ., data = train,kernel = "radial", cost = 5, gamma = 0.001)
#svm.radial
...
# Fit a model
train.full$death <- as.factor(train.full$death)
levels(train.full$death) <- c("No", "Yes")
glmnet.cv1 <- train(death ~ ., train.full, method = "glmnet",
...
# glmnet.cv1
glmnet.cv1$bestTune
save(glmnet.cv1, file = "glmnet.cv1.RData")
```

```{r}
glmnet.fit <- glmnet(x.train.full, y.train, family = "binomial", 
                     alpha = 0.4, lambda = 0.01)
glmnet.pred <- predict(glmnet.fit, newx = x.valid.full, s = 0.01, type = "response")
```


```{r fig.height=4, fig.width=4}
glmnet.roc <- rocit(score = glmnet.pred[,1], class = y.valid)
plot(glmnet.roc, values = T, YIndex = F, legend = F)
legend("bottomright", legend = paste0("AUC = ", round(glmnet.roc$AUC, 3)), 
       bty = "n")
```
...
boost.test.outcomes=ifelse(boost.test.probs>0.5, TRUE, FALSE)
#make final df with patient id and prediction
final_preds = data.frame(patient_id = test_df[,c(1)], boost.pred= boost.test.probs, mortality.pred = boost.test.outcomes)
#write to csv
write.csv(final_preds,'final_preds.csv', row.names = FALSE)
```
